# NEVOLYN Technology — Master CI/CD, Database & Deployment Handbook
...
```yaml
  db:
    ...
    container_name: nevolyn_db
    environment:
      POSTGRES_DB: ${DB_NAME:-nevolyn_db}
      POSTGRES_USER: ${DB_USER:-nevolyn_admin}
      POSTGRES_PASSWORD: ${DB_PASS:-Nevolyn_Secure_Pass_2026!}
    ports:
      - "5432:5432"
    volumes:
...
| **Database Name** | `nevolyn_db` | `nevolyn_db` |
| **Username** | `sa` | `nevolyn_admin` (or `${DB_USER}`) |
| **Password** | *(No password)* | `Nevolyn_Secure_Pass_2026!` (or `${DB_PASS}`) |
...
When you acquire your custom domain name (e.g. `nevolyn.com` for Frontend and `api.nevolyn.com` for Backend), follow this simple 5-step checklist:
...
| :--- | :--- | :--- | :--- |
| **A Record** | `@` (or `www`) | `YOUR_SERVER_IP` | `nevolyn.com` (Frontend) |
| **A Record** | `api` | `YOUR_SERVER_IP` | `api.nevolyn.com` (Backend) |
...
```bash
git clone https://github.com/mninadmnobo/nevolyn_official_website.git
cd nevolyn_official_website
```
...
```text
 ...
 │ - Point A Records for nevolyn.com & api.nevolyn.com to VPS   │
...
```text
 ...
 │ nevolyn.vercel.app│         CORS Allowed     └──────────┬───────────┘
 └──────────────────────┘                                     │
                                                         PostgreSQL
                                                              ▼
                                                   ┌──────────────────────┐
...
3. Connect your GitHub repository (`nevolyn_official_website`).
4. Render will automatically detect `render.yaml` at the root of the project:
   - **Database**: `nevolyn-db` (PostgreSQL 16)
   - **Web Service**: `nevolyn-backend` (Built via `backend/Dockerfile`)
5. Click **Apply**.
6. Once deployed, copy your backend URL (e.g. `https://nevolyn-backend.onrender.com`).
...
3. Import your GitHub repository (`nevolyn_official_website`).
4. Configure Project Settings:
   - **Framework Preset**: Next.js
   - **Root Directory**: Select `frontend`
5. Expand **Environment Variables** and add:
   - `NEXT_PUBLIC_API_URL`: `https://nevolyn-backend.onrender.com` (Your Render backend URL)
6. Click **Deploy**. Vercel will build and assign your site URL (e.g. `https://nevolyn.vercel.app`).
